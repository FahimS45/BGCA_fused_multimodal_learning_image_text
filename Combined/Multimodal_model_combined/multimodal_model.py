"""Complete Multimodal Model with Gated Cross-Attention for Combined Dataset."""
...
class MultiModalModelGatedCrossAttention(nn.Module):
    ...
    Updated for combined dataset (DMID + INbreast + MIAS).
    """
    
    def __init__(self, resnet_model: nn.Module, deit_model: nn.Module, 
                 text_model: nn.Module, fc_network: FullyConnectedNetwork,
                 resnet_dim: int = 1024, deit_dim: int = 768, 
                 text_dim: int = 768, hidden_dim: int = 512):
        """
        Initialize the Multimodal Model.
        
        Args:
            resnet_model: Pre-trained DenseNet121 model (named resnet for compatibility)
            ...
            resnet_dim: DenseNet feature dimension (1024 for DenseNet121)
        ...
        self.resnet_model = resnet_model  # Actually DenseNet121
        self.deit_model = deit_model
        self.text_model = text_model
        
        # Store dimensions
        self.resnet_dim = resnet_dim  # DenseNet121 classifier input features = 1024
    ...
    def forward(self, image_input: torch.Tensor, input_ids: torch.Tensor, 
        ...
        densenet_features = self.resnet_model(image_input)       # [B, 1024] - DenseNet121
        deit_features = self.deit_model(image_input).logits      # [B, 768] - DeiT
        
        # Concatenate vision features
        vision_features = torch.cat([densenet_features, deit_features], dim=1)  # [B, 1792]
