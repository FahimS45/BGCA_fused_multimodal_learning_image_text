"""Results analysis and summarization for MIAS classification."""
...
def bootstrap_ci(data, n_bootstrap: int = 10000, ci: int = 95):
...
def summarize_classification_results(test_reports: List[Dict], test_auc_roc: List[float], 
    ...
    # Calculate summary statistics with bootstrap CI
    summary_rows = []
    for metric in metrics_df.columns:
        mean_val = metrics_df[metric].mean()
        std_val = metrics_df[metric].std()
        boot_mean, ci_lower, ci_upper = bootstrap_ci(
            metrics_df[metric].dropna().values, n_bootstrap, ci
        )
